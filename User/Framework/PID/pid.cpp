...
{//最后四个参数初定为   ,1.0f,30000.0f,2000.0f,300.0f
    //初始化PID的默认参数
    WhichPID->Kp = Kp;
    WhichPID->Ki = Ki;
    WhichPID->Kd = Kd;

    WhichPID->PID_Precision = PID_Precision;
    WhichPID->PID_ErrAllMax = PID_ErrAllMax;
    WhichPID->PID_OutMax = PID_OutMax;
    WhichPID->PID_OutStep = PID_OutStep;
}
...
float PIDc::Pos_Cascade_PID(PID_t *Spdparam,PID_t *Posparam, float Pos_Target, float Pos_feedback, const float Spd_feedback)
{
    //01:
